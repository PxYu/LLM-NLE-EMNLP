    ...
    def rescore(self, query: Query, texts: List[Text]) -> List[Text]:
        ...
        for batch in self.tokenizer.traverse_query_document(batch_input):
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                ...
                decode_ids, next_token_logits, model_outputs = greedy_decode(self.model,
                                                input_ids,
                                                length=1,
                                                attention_mask=attn_mask,
                                                return_last_logits=True)
                
                batch_decoder_last_hidden_states = []
                for i in range(model_outputs['decoder_hidden_states'][-1].shape[0]):
                    batch_decoder_last_hidden_states.append(model_outputs['decoder_hidden_states'][-1][i, :, :].squeeze().cpu().numpy())

                batch_logits = next_token_logits[:, [self.token_false_id, self.token_true_id]]
                batch_scores_2_options = torch.nn.functional.softmax(batch_logits, dim=1)
                batch_probs = batch_scores_2_options[:, 1].tolist()
                batch_logits = batch_logits.cpu().numpy().tolist()
            # for doc, score in zip(batch.documents, batch_log_probs):
            for doc, score, logits, last_hidden_state in zip(batch.documents, batch_probs, batch_logits, batch_decoder_last_hidden_states):
                doc.score = score
                doc.logits = logits
                doc.last_hidden_states = last_hidden_state
...
class MonoBERT(Reranker):
    def __init__(self,
                 pretrained_model_name_or_path: str  = 'castorini/monobert-large-msmarco',
    ...
    def rescore(self, query: Query, texts: List[Text]) -> List[Text]:
        ...
        for text in texts:
            ...
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                ...
                output = self.model(input_ids, token_type_ids=tt_ids, return_dict=True, output_hidden_states=True)
                last_hidden_state = output.hidden_states[-1]
                logits = output.logits
                if logits.size(1) > 1:
                    # text.score = torch.nn.functional.log_softmax(
                    #     output, 1)[0, -1].item()
                    text.logits = logits[0].cpu().numpy().tolist()
                    text.score = torch.nn.functional.softmax(
                        logits, 1)[0, -1].item()
                    text.last_hidden_states = last_hidden_state[0, 0].cpu().numpy()
                else:
